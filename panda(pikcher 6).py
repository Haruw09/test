...
def panda(x, y, size, scr):  # x,y -координаты, size - размер, scr -пространство
    ...
    polygon(scr, black, [[x - size / 10, y + size / 2], [x - size / 5, y + 1.3 * size], [x - size / 2, y + 1.58 * size],
                         [x - 0.8 * size, y + 1.28 * size]])
    b_ell(x - size * 0.57, y + size * 0.45, size * 0.37, size, -18, black)
    b_ell(x - 0.95 * size, y + 0.95 * size, 0.45 * size, 0.7 * size, -52, black)
    polygon(scr, black, [[x - 0.8 * size, y], [x - 0.8 * size, y + size], [x - size, y + 1.3 * size],
                         [x - 1.3 * size, y + 1.5 * size], [x - 1.4 * size, y + 1.1 * size]])
    b_ell(x - 1.7 * size, y + 0.95 * size, size * 0.4, size * 0.6, -55, black)
    b_ell(x - 2.2 * size, y + 0.3 * size, size * 0.4, size * 1.1, -30, black)

    # голова
    polygon(scr, white, [[x - 1.2 * size, y - 0.3 * size], [x - 0.9 * size, y], [x - 0.9 * size, y + 0.8 * size],
                         [x - 1.65 * size, y + size], [x - 1.9 * size, y]])
